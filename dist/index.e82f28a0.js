...
window.addEventListener("DOMContentLoaded", ()=>{
    ...
    async function renderInputs(bank = "BB", type = "sell") {
        ...
        while(sectionMain.hasChildNodes())sectionMain.removeChild(sectionMain.lastChild);
        let informer;
        if (bank === "BB" && type === "sell") {
            informer = new _bbcardsSellInputs.BBCardsSellInputs();
            ...
            informer = new _bbcardsBuyInputs.BBCardsBuyInputs();
            sectionMain.append(informer.render());
            let dataCards = await cardBB.getCardCursFromApi();
    ...
    async getCardCursFromApi() {
        try {
            ...
            if (req.ok) {
                ...
                const res = {};
                for(let key in json[0])this.currencyArray.forEach((item)=>{
                    if (key === item) res[key] = json[0][key];
                });
                return res;
    ...
    async getCardConversionFromApi() {
        try {
            ...
            if (req.ok) {
                const json = await req.json();
                const res = {};
                for(let key in json[0])this.conversionArray.forEach((item)=>{
                    if (key === item) res[key] = json[0][key];
                });
                return res;
            } else throw new Error(`${req.status}`);
